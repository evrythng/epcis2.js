...
describe('unit tests for the ObjectEvent class', () => {
  ...
  it('setters should set the variables correctly', async () => {
    ...
    expect(obj.getErrorDeclaration().getDeclarationTime()).to.be.equal(JSONObjectEvent.errorDeclaration.declarationTime)
    expect(obj.getErrorDeclaration().getReason()).to.be.equal(JSONObjectEvent.errorDeclaration.reason)
    expect(obj.getErrorDeclaration().getCorrectiveEventIDs().toString()).to.be.equal(JSONObjectEvent.errorDeclaration.correctiveEventIDs.toString())
    expect(obj.getErrorDeclaration()['example:vendorExtension']).to.be.equal(JSONObjectEvent.errorDeclaration['example:vendorExtension'])
    ...
    expect(obj.getPersistentDisposition().getUnset().toString()).to.be.equal(JSONObjectEvent.persistentDisposition.unset.toString())
    expect(obj.getPersistentDisposition().getSet().toString()).to.be.equal(JSONObjectEvent.persistentDisposition.set.toString())
    expect(obj.getReadPoint().getId()).to.be.equal(JSONObjectEvent.readPoint.id)
    expect(obj.getIlmd()['example:bestBeforeDate']).to.be.equal(JSONObjectEvent.ilmd['example:bestBeforeDate'])
  })
  it('should create an ObjectEvent from json', async () => {
    const obj = new ObjectEvent(JSONObjectEvent)
    console.log(obj)
    console.log(obj.epcList)
    expect(obj.getEPCList().toString()).to.be.equal(JSONObjectEvent.epcList.toString())
    expect(obj.getEventID()).to.be.equal(JSONObjectEvent.eventID)
    expect(obj.getEventTime()).to.be.equal(JSONObjectEvent.eventTime)
    expect(obj.getEventTimeZoneOffset()).to.be.equal(JSONObjectEvent.eventTimeZoneOffset)
    ...
    expect(obj.getBizTransactionList().length).to.be.equal(JSONObjectEvent.bizTransactionList.length)
    expect(obj.getBizTransactionList()[0].bizTransaction).to.be.equal(JSONObjectEvent.bizTransactionList[0].bizTransaction)
    expect(obj.getDestinationList().length).to.be.equal(JSONObjectEvent.destinationList.length)
    expect(obj.getDestinationList()[0].destination).to.be.equal(JSONObjectEvent.destinationList[0].destination)
    expect(obj.getSensorElementList().length).to.be.equal(JSONObjectEvent.sensorElementList.length)
    expect(obj.getSensorElementList()[0].sensorMetadata.bizRules).to.be.equal(JSONObjectEvent.sensorElementList[0].sensorMetadata.bizRules)
    expect(obj.getSensorElementList()[1].sensorReport[0].minValue).to.be.equal(JSONObjectEvent.sensorElementList[1].sensorReport[0].minValue)
    expect(obj.getIlmd()['example:bestBeforeDate']).to.be.equal(JSONObjectEvent.ilmd['example:bestBeforeDate'])
  })
