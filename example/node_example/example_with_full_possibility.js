...
const buildIlmdExample = () => {
  return new Ilmd().addExtension("ext1:boolean","true",)
  .addExtension("ext1:value",Math.floor(Math.random() * 9));
}

const buildObjectEvent = () => {
...
const sendACaptureRequestExample = async () => {
  try {
    ...
    console.log('\nAssociationEvent is valid ? ' + epcisDocument.getEventList()[0].isValid());
    console.log('ObjectEvent is valid ? ' + epcisDocument.getEventList()[1].isValid());
    console.log('ExtendedEvent is valid ? ' + epcisDocument.getEventList()[2].isValid());
